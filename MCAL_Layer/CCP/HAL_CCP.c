...
Std_ReturnType CCP_Init(const ccp_t *_ccp_obj){
    ...
    if(NULL == _ccp_obj){
        ...
        /* PIN Configurations */
        ret = GPIO_pin_inti(&(_ccp_obj->ccp_pin));
...
static void CCP_PWM_Mode_Config(const ccp_t *_ccp_obj){
    ...
    PR2 = (u8)(((float)_XTAL_FREQ / ((float)_ccp_obj->PWM_Frequency * 4.0 * (float)_ccp_obj->timer2_prescaler_value * 
...
/* Interrupt Priority Configurations */
    ...
    if(INT_HIGH_PRIORITY == _ccp_obj->CCP1_priority){
        /* Enables all high-priority interrupts */
        INT_GlobalInterruptHighEnable();
        CCP1_HighPriorityset();
    }
    else if(INT_LOW_PRIORITY == _ccp_obj->CCP1_priority){
...
/* Interrupt Priority Configurations */
#if INT_PRIORITY_LEVELS_ENABLE==INTERRUPT_FEATURE_ENABLE 
    INT_PRIORITYLevelsEnable();
    if(INT_HIGH_PRIORITY == _ccp_obj->CCP2_priority){
        /* Enables all high-priority interrupts */
        INT_GlobalInterruptHighEnable();
        CCP2_HighPriorityset();
    }
    else if(INT_LOW_PRIORITY == _ccp_obj->CCP2_priority){
        /* Enables all unmasked peripheral interrupts */
        INT_GlobalInterruptLowEnable();
        CCP2_LowPriorityset();
    }
